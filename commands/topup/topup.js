...
\`Layanan game\`
${quote(`Format order: ${config.bot.prefix}topup provider_id user_id`)}
${quote(`Contoh: ${config.bot.prefix}topup 1124 2272144724`)}

\`Khusus Layanan game yang memakai Zone ID/Server\`
${quote(`Format order: ${config.bot.prefix}topup provider_id user_id zone_id`)}
${quote(`Contoh: ${config.bot.prefix}topup 1000 877851782 10683`)}
`;	
		...
		if (fs.existsSync(tmpFilePath)) {
			...
			try {
				...
				.then(response => {
					...
					const produk = data.find(item => item.provider_id == provider);
					...
					performDeposit(produkDetail, finalPrice, provider, target, produk_kode, produk_name);
				})
				.catch(error => {
				...
				function performDeposit(product, nominal, provider, target, produk_kode, produk_name) {
...
*Note:* Pembayaran akan otomatis dibatalkan oleh bot dalam 5 menit lagi!`;
					...
					checkPaymentStatus(data.data.reference, data.data.invoice_id, provider, target, produk_kode, produk_name);
				...
				function checkPaymentStatus(payId, reffId, provider, target, produk_kode, produk_name) {
					...
					const interval = setInterval(() => {
						...
						.then(response => {
							...
							if(data.code === "x_args") {
								...
								performTopupTransaction(reffId, provider, target, produk_kode, produk_name);
				...
				function performTopupTransaction(reffId, provider, target, produk_kode, produk_name) {
